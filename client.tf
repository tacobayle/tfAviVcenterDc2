resource "vsphere_tag" "ansible_group_client" {
  name             = "${var.vcenter.prefix}-client"
...
data "template_file" "client_userdata" {
  ...
  vars = {
    ...
    avi_dns_vs = split("-", var.avi.config.cloud.networks[2].avi_ipam_pool)[0]
  }
}
...
resource "null_resource" "traffic_gen_copy" {
  ...
  provisioner "file" {
    source      = "json/loopback_ips.json"
    destination = "loopback_ips.json"
  }

  provisioner "file" {
    source      = "json/user_agents.json"
