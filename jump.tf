...
resource "null_resource" "config_jump" {
  depends_on = [vsphere_virtual_machine.jump]
  provisioner "local-exec" {
    command = "/bin/bash bash/config_jump.sh ${vsphere_virtual_machine.jump.default_ip_address} ${var.jump.private_key_path}"
  }
}